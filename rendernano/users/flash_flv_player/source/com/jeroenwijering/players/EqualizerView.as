...
class com.jeroenwijering.players.EqualizerView extends AbstractView { 
	...
	function EqualizerView(ctr:AbstractController,car:Object,far:Array) { super(ctr,car,far); };


	/** onLoad Override: setup EQ **/
	private function onLoad() {
		eqClip = configArray["playerclip"].display.equalizer;
		eqClip._y = configArray["displayheight"] - 50;
		eqStripes = Math.floor((configArray['width'] - 20)/6);
		eqClip.stripes.duplicateMovieClip("stripes2",1);
		eqClip.mask.duplicateMovieClip("mask2",3);
		eqClip.stripes._width = eqClip.stripes2._width = configArray['width'] - 20;
		eqClip.stripes.top.col = new Color(eqClip.stripes.top);
		eqClip.stripes.top.col.setRGB(configArray['lightcolor']);
		eqClip.stripes.bottom.col = new Color(eqClip.stripes.bottom);
		eqClip.stripes.bottom.col.setRGB(configArray['backcolor']);
		eqClip.stripes2.top.col = new Color(eqClip.stripes2.top);
		eqClip.stripes2.top.col.setRGB(configArray['lightcolor']);
		eqClip.stripes2.bottom.col = new Color(eqClip.stripes2.bottom);
		eqClip.stripes2.bottom.col.setRGB(configArray['backcolor']);
	...
	private function setState(stt:Number) { stt == 2 ? eqClip._visible = true: eqClip._visible = false; };
