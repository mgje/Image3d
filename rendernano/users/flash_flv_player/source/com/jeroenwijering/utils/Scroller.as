...
class com.jeroenwijering.utils.Scroller {
	...
	**/
	function Scroller(tgt:MovieClip,msk:MovieClip,asc:Boolean,fcl:Number,hcl:Number) {
		targetClip = tgt;
		maskClip = msk;
		...
		autoScroll == false ? drawScrollbar(): scrollInterval = setInterval(this,"doAutoscroll",50);
	...
	private function drawScrollbar():Void {
		...
		SCROLLER_CLIP._x = maskClip._x+maskClip._width;
		...
		Draw.square(SCROLLER_CLIP.front,6,SCROLLER_CLIP.bar._height*sizeRatio,frontColor);
		SCROLLER_CLIP.front.createEmptyMovieClip("bg",1);
		SCROLLER_CLIP.front.bg._x = -3;
		SCROLLER_CLIP.front.bg._alpha = 0;
		Draw.square(SCROLLER_CLIP.front.bg,12,SCROLLER_CLIP.front._height,frontColor);
	...
	public function onMouseWheel(delta):Void { scrollTo(currentScroll-delta*20); };


	/** Set autoscroll events. **/
	private function doAutoscroll():Void {
		if (maskClip._xmouse>0 && maskClip._xmouse<maskClip._width/(maskClip._xscale/100) && 
			maskClip._ymouse>0 && maskClip._ymouse<maskClip._height/(maskClip._yscale/100)) {
			var dif:Number = maskClip._ymouse*(maskClip._yscale/100)-maskClip._height/2;
